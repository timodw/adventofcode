...
int main() {
    ...
    std::unordered_map<int, std::string> pos_permutations;
    for (size_t i = 0; i < 1000000000; i++) {
        ...
        if (permutations.find(positions) != permutations.end()) {
            std::cout << pos_permutations[(999999999 - i) % i];
            break;
        } else {
            permutations.insert(positions);
            pos_permutations[i] = positions;
        }
    }
    return 0;
}
